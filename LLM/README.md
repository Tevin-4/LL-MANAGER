...
This repository includes a `render.yaml` Blueprint specification file to automate deployment on **Render** (Flask API + Static Site Frontend + PostgreSQL Database).
...
5. Render will automatically detect the `render.yaml` file and prompt you to create the services:
   * **football-league-api** (Flask Backend Web Service)
   * **football-league-frontend** (Static Site Frontend)
   * **football-league-db** (Managed PostgreSQL Database)
6. Once the backend deployment completes, copy its live URL (e.g. `https://football-league-api.onrender.com`).
7. Update `BASE_URL` in `frontend/js/api.js` to point to your backend:
   ```javascript
   const BASE_URL = "https://your-api-name.onrender.com/api";
   ```
8. Commit and push the `api.js` change to GitHub. Render will automatically redeploy the static frontend with the live API connection!
